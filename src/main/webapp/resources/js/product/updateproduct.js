...
//	var productId = 8;
	var getProductCategoryUrl = '/SSMshop/productcategoryadmin/getproductcategorylist?shopId='
			+ shopId;
	var getProductInfoUrl = '/SSMshop/productoperation/getproductbyid?productId='
			+ productId;
	...
	function getInitInfo() {
		$
				.getJSON(
						...
						function(data) {
							if (data.success) {
								var productCategoryId = data.product.productCategory.productCategoryId;
								$('#product-name').val(data.product.productName)
								$('#normal-price').val(data.product.normalPrice)
								$('#promotion-price').val(data.product.promotionPrice)
								$('#priority').val(data.product.priority)
								$('#product-desc').val(data.product.productDesc)
								$('#product-category')
										.find(
												'option[value="'
														+ productCategoryId
